...
void TopoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  // waypoint_sub_ = nh.subscribe("/waypoint_generator/waypoints", 1, &TopoReplanFSM::waypointCallback, this);
      //by zhou set A to set waypoints
  waypoint_sub_ = nh.subscribe("/mavros/rc/in",1, &TopoReplanFSM::rcin_callback, this);
...
void TopoReplanFSM::rcin_callback(const mavros_msgs::RCIn & rcvalue)
{
  ...
  if(channel7_value <1600)
  {
    channel7_flag = false;
  }

  if(channel7_value >1800 && channel7_flag==false)
  {
    channel7_flag = true;
    ...
    if (target_type_ == TARGET_TYPE::REFENCE_PATH) {
      ...
      // }
      for (int i = inorout; i < inorout+3; ++i) {
        Eigen::Vector3d pt;
        pt(0) = waypoints_[i][0];
        pt(1) = waypoints_[i][1];
        pt(2) = waypoints_[i][2];
